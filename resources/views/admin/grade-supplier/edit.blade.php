...
@section('title', 'Edit Grade Supplier')
    ...
    <div class="w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-6">
            <div class="flex items-center mb-2">
                <a href="{{ route('grade-supplier.index') }}" class="text-gray-600 hover:text-gray-900 mr-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                    </svg>
                </a>
                <h1 class="text-2xl font-semibold text-gray-800">Edit Grade Supplier</h1>
            </div>
            <p class="text-sm text-gray-600 ml-7">Perbarui data grade supplier di bawah ini</p>
        </div>

        <!-- Form Card -->
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm">
            <form action="{{ route('grade-supplier.update', $gradeSupplier->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <input type="text"
                           name="name"
                           id="name"
                           value="{{ old('name', $gradeSupplier->name) }}"
                ...
                <div class="mb-6">
                    ...
                    @if($gradeSupplier->image_url)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $gradeSupplier->image_url) }}" alt="Gambar Supplier" class="h-24 rounded shadow">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description"
                              ...
                              placeholder="Masukkan deskripsi supplier (opsional)">{{ old('description', $gradeSupplier->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
