...
@section('title', 'Edit Parent Grade Company')
        ...
        <div class="w-full px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="mb-6">
                <div class="flex items-center mb-2">
                    <a href="{{ route('parent-grade-companies.index') }}" class="text-gray-600 hover:text-gray-900 mr-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                        </svg>
                    </a>
                    <h1 class="text-2xl font-semibold text-gray-800">Edit Parent Grade Company</h1>
                </div>
                <p class="text-sm text-gray-600 ml-7">Perbarui data parent grade company di bawah ini</p>
            </div>

            <!-- Form Card -->
            <div class="bg-white border border-gray-200 rounded-lg shadow-sm">
                <form action="{{ route('parent-grade-companies.update', $parentGradeCompany->id) }}" method="POST"
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $parentGradeCompany->name) }}"
                    ...
                    <div class="mb-6">
                        ...
                        @if($parentGradeCompany->image_url)
                            <div class="mb-3">
                                <img src="{{ asset('storage/' . $parentGradeCompany->image_url) }}" alt="Gambar Parent Grade"
                        ...
                        @error('image_url')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-gray-500">Kosongkan jika tidak ingin mengganti gambar. Format: JPG,
                            JPEG, PNG (maks. 2MB)</p>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="5"
                            ...
                            placeholder="Masukkan deskripsi (opsional)">{{ old('description', $parentGradeCompany->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
