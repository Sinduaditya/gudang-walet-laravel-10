...
@section('title', 'Edit Sortir Bahan')

@section('content')
    <div class="bg-white min-h-screen">
        <div class="w-full px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-semibold text-gray-800">Edit Sortir Bahan</h1>
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                <form action="{{ route('sort-materials.update', $sortMaterial->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="sort_date" id="sort_date"
                            value="{{ old('sort_date', $sortMaterial->sort_date->format('Y-m-d')) }}" required
                    ...
                    <div class="mb-4">
                        ...
                        <select name="parent_grade_company_id" id="parent_grade_company_id" required
                            ...
                            @foreach ($parentGradeCompanies as $pg)
                                <option value="{{ $pg->id }}" {{ old('parent_grade_company_id', $sortMaterial->parent_grade_company_id) == $pg->id ? 'selected' : '' }}>
                                    {{ $pg->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" step="0.01" name="weight" id="weight"
                            value="{{ old('weight', $sortMaterial->weight) }}" required
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('description', $sortMaterial->description) }}</textarea>
                    </div>
    ...
    @push('scripts')
        <script>
            ...
            const initialParentId = "{{ old('parent_grade_company_id', $sortMaterial->parent_grade_company_id) }}";
            const initialGradeId = "{{ old('grade_company_id', $sortMaterial->grade_company_id) }}";

            // Initialize dropdowns
            document.addEventListener('DOMContentLoaded', function () {
                if (initialParentId) {
                    updateGradeCompanyDropdown(initialParentId, initialGradeId);
                }
            });
