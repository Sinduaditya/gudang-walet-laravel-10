        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col md:flex-row gap-6 mb-8 items-start">
                ...
                <div class="w-64 bg-white border border-gray-200 rounded-2xl p-4 shadow-sm flex-shrink-0">
                    <div
                        class="relative w-full aspect-square bg-black rounded-xl mb-4 flex items-center justify-center overflow-hidden group shadow-inner">
                    ...
                    <form method="GET" action="{{ route('tracking-stock.detail', $grade->id) }}">
                        <div class="flex flex-col lg:flex-row gap-4">
                            ...
                            <div class="flex items-end gap-2">
                                <button type="submit"
                                    class="px-4 py-2 bg-blue-600 text-white rounded-md text-sm">Cari</button>
                                <a href="{{ route('tracking-stock.detail', $grade->id) }}"
                                    class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md text-sm">Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Stok (Gram)
                                </th>
                                <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase w-24">Status
                                </th>
                            </tr>
                        </thead>
                            ...
                            @forelse($locationStocks as $stock)
                                ...
                                <tr class="location-row hover:bg-blue-50 transition-colors duration-150"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <span
                                            class="text-sm font-bold text-gray-900">{{ number_format($stock->total_stock, 0, ',', '.') }}</span>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        ...
                                        @else
                                            <span
                                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Empty</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr id="noDataRow">
                                    <td colspan="5" class="px-6 py-12 text-center text-sm text-gray-500">Tidak ada data
                                        stok.</td>
                                </tr>
                            @endforelse
                            <tr id="filterNoData" class="hidden">
                                <td colspan="5" class="px-6 py-12 text-center text-sm text-gray-500">Tidak ada supplier
                                    yang sesuai filter.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
