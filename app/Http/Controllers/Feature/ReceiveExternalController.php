...
class ReceiveExternalController extends Controller
{
    ...
    public function receiveExternalStep1(Request $request)
    {
        try {
            ...
            Log::error('Error in receiveExternalStep1: ' . $e->getMessage(), [
                'user_id' => auth()->id(),
    ...
    public function checkExternalStock(Request $request)
    {
        try {
            ...
            Log::error('Error in checkExternalStock: ' . $e->getMessage(), [
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString()
            ]);
            return response()->json([
    ...
    public function storeReceiveExternalStep1(Request $request)
    {
        try {
            ...
            Log::error('Error in storeReceiveExternalStep1: ' . $e->getMessage(), [
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function receiveExternalStep2()
    {
        try {
            ...
            Log::error('Error in receiveExternalStep2: ' . $e->getMessage(), [
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString()
            ]);
            return redirect()->route('barang.keluar.receive-external.step1')
    ...
    public function receiveExternal(Request $request)
    {
        try {
            ...
            Log::error('Error in receiveExternal: ' . $e->getMessage(), [
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString()
            ]);
            return redirect()->back()->with('error', 'Terjadi kesalahan saat memproses penerimaan.');
        }
    }
