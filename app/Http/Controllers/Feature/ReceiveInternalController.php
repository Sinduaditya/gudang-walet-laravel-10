...
class ReceiveInternalController extends Controller
{
    ...
    public function receiveInternalStep1(Request $request)
    {
        try {
            ...
            Log::error('Error in receiveInternalStep1: ' . $e->getMessage(), [
                'user_id' => auth()->id(),
    ...
    public function checkInternalStock(Request $request)
    {
        try {
            ...
            Log::error('Error in checkInternalStock: ' . $e->getMessage(), [
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString()
            ]);
            return response()->json([
    ...
    public function storeReceiveInternalStep1(Request $request)
    {
        try {
            ...
            Log::error('Error in storeReceiveInternalStep1: ' . $e->getMessage(), [
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function receiveInternalStep2()
    {
        try {
            ...
            Log::error('Error in receiveInternalStep2: ' . $e->getMessage(), [
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString()
            ]);
            return redirect()->route('barang.keluar.receive-internal.step1')
    ...
    public function receiveInternal(Request $request)
    {
        try {
            ...
            Log::error('Error in receiveInternal: ' . $e->getMessage(), [
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString()
            ]);
            return redirect()->route('barang.keluar.receive-internal.step1')
                ->with('error', 'Terjadi kesalahan saat memproses penerimaan barang.');
        }
    }
}